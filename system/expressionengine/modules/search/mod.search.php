...
class Search {
  ...
  var	$min_length		= 2;			// Minimum length of search keywords
  ...
  var $num_rows		= 0;
  /**
   * CUSTOM ADDED BY ANDREAS FOR WEIGHT COLUMN SEARCH
   * @since 2015-08-27
   * @author  Andreas Bilz
   * @description Taken from EE 2.8 where this was introduced
   */
  var $hash			= "";
  ...
  function do_search()
  {
    ...
    if ($sql == FALSE)
    {
      ...
      {
        /**
         * COMMENTED BY ANDREAS FOR WEIGHT COLUMN SEARCH
         * @since 2015-08-27
         * @author  Andreas Bilz
         * @description Taken from EE 2.8 where this was introduced
         */
//				$this->hash = ee()->functions->random('md5');
  ...
  protected function _get_meta_vars()
  {
    ...
    else
    {
      ...
      $this->hash = substr($raw, -32);
      $meta_array = substr($raw, 0, -32);

      if ($this->hash != md5(ee()->db->username.ee()->db->password.$meta_array))
      {
        $meta_array = '';
      }
    }
  ...
  function build_standard_query()
  {
    ...
    /**
     * CUSTOM ADDED BY ANDREAS FOR WEIGHTED SEARCH EXTENSION
     * @since 2015-08-27
     * @author  Andreas Bilz
     * @description Taken from EE 2.8 where this was introduced
     */
    ...
    {
      $this->EE->logger->developer($this->hash);
  ...
  function search_results()
  {
    ...
    /**
     * CUSTOM ADDED BY ANDREAS FOR WEIGHTED SEARCH EXTENSION
     * AS THIS IS NOT AVAILABLE IN 2.6.1
     * @since 2015-08-27
     * @author  Andreas Bilz
     * @description Taken from EE 2.8 where this was introduced
     */

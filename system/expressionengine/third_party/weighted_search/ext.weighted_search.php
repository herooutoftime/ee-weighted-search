...
class Weighted_search_ext {
        ...
        foreach($fields as $key => $value) {
            switch($key) {
                ...
                default:
                    ...
                    $vars['fields'][$key] = array('i', '', $value['value'], $value['name']);
            }
        }
    ...
    public function get_channels()
    {
        ...
        foreach($query->result() as $row) {
            ...
            $channels['channel_id_' . $row->channel_id] = array('value' => $value, 'name' => $row->channel_name);
        }
        return $channels;
    }
    ...
    public function get_fields()
    {
        ...
        foreach($query->result() as $row) {
            ...
            $fields['field_id_' . $row->field_id] = array('value' => $value, 'name' => $row->field_name);
        }
        return $fields;
    }
    ...
    public function prepare_fields($vars)
    {
        foreach ($vars['fields'] as $key => $options)
        {
            ...
            $vars['fields'][$key] = array('type' => $options[0], 'value' => $details, 'subtext' => $sub, 'selected' => $selected, 'name' => $options[3]);
        }
        return $vars;
    }
